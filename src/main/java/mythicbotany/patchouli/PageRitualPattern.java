...
public class PageRitualPattern extends PageRuneRitualBase {
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.recipe != null) {
            this.addRune(poseStack, this.recipe.getCenterRune(), 0, 0, true, mouseX, mouseY);
            for (RuneRitualRecipe.RunePosition rune : this.recipe.getRunes()) {
                this.addRune(poseStack, rune.getRune(), rune.getX(), rune.getZ(), rune.isConsumed(), mouseX, mouseY);
            }
        }
    }
    ...
    private void addRune(PoseStack poseStack, Ingredient rune, int x, int z, boolean consume, int mouseX, int mouseY) {
        ...
        if (stacks.length > 0) {
            ...
            RenderHelper.transferMsToGl(poseStack, () -> {
                ...
                Minecraft.getInstance().getItemRenderer().renderGuiItemDecorations(this.parent.font, stack, realX, realY);
            });
            poseStack.popPose();
            if (this.parent.isMouseInRelativeRange(mouseX, mouseY, realX, realY, 10, 10)) {
                ...
                this.parent.drawCenteredStringNoShadow(poseStack, text1, 0, 0, color1);
                this.parent.drawCenteredStringNoShadow(poseStack, text2, 0, 2 + this.parent.font.lineHeight, color2);
                poseStack.popPose();
            }
        }
    }

...
public class RenderAlfsteelPylon extends TileEntityRenderer<TileAlfsteelPylon> {
    ...
    public static final RenderType PYLON_GLOW = initFieldPylonGlow();

    private static RenderType initFieldPylonGlow() {
        ...
        RenderType layer = RenderType.makeType(MythicBotany.MODID  + ":alfsteel_pylon_glow", DefaultVertexFormats.ENTITY, 7, 128, glState.build(false));
    ...
    public void doRender(@Nullable TileAlfsteelPylon pylon, float pticks, MatrixStack ms, IRenderTypeBuffer buffers, int light, int overlay) {
        ...
        buffer = buffers.getBuffer(PYLON_GLOW);
